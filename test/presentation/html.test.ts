...
test('class content renders the legacy minutes-until-bell header contract', () => {
  ...
  assert.match(
    inClass,
    /<audio data-water-break-end-tone preload="auto" src="\/assets\/water-break-end\.wav"><\/audio>/u,
  );
...
test('dismissal scene defaults to the tidy-area instruction and promotes its countdown', () => {
  ...
  assert.match(html, /<p class="eyebrow">Dismissing soon<\/p>/u);
  assert.doesNotMatch(html, /Dismissal (?:begins|starts) soon/u);
