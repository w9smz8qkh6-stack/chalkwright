...
defines security invariants and downstream negative-test obligations without
accepting [ADR-0026](decisions/0026-public-core-and-hosted-shell.md), selecting
vendors, or authorizing package, provider, repository, infrastructure, billing,
route, service, publication, deployment, or other live changes.
...
| ---------------------------------------------------------------------------------------------------- | -------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- | --------------------------------------------------------------------------------------------------------- |
| Core has no account/login and equates private operator reachability with full installation authority | Deliberate Moderate residual. Safe private defaults, clear warning, origin/host controls, and publication tests are mandatory; public-by-default behavior is blocked.                                                                                                      | Listener/process/package mechanism in `A03`; implementation/evidence in `B05`, `B07`, `C02`, `C12`, `F04` |
| Class codes are shareable low-entropy convenience credentials                                        | Deliberate Moderate residual only for a minimal screen-safe projection. Code/session parameters and atomic revocation remain undecided.                                                                                                                                    | Persistence/thresholds in `A05`; fixtures in `A08`; implementation in `C03`/`D10`; privacy in `E01`/`F01` |
...
| Shared Core supply chain and cross-repository compatibility                                          | Moderate upstream/dependency residual. Exact pins, provenance, restricted exports, compatibility, migration, and rollback gates are mandatory.                                                                                                                             | Mechanism in `A03`; `B01`/`B02`/`B06`/`B07`; hosted gates in `D01`/`D12`/`E04`                            |
...
| Concrete auth library, schema, thresholds, infrastructure, and vendor choices                        | Intentionally unresolved, not silently accepted. Each later task must preserve these invariants and record its own version-correct threat impact.                                                                                                                          | `A03`-`A08`, `D02`, `D11`, `F01`-`F05` as assigned in the work breakdown                                  |

## A02 completion and next gate
...
A03 is next. It must resolve the security-significant package/artifact,
one-process/two-listener versus separate-process, compatibility-window, and
shared-presentation choices in ADR-0026 against this model. A04 through A08
remain incomplete, Phase B has not begun, and no runtime or live effect occurred
in A02.
