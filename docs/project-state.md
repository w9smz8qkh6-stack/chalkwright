...
- Implementation files covered: 541
- Implementation fingerprint: `695768fbf07fb9f5d3a0ef002fa30f4a2eb7f017a9eaf2c1a631f82cbb27a798`
- Semantic review: **CURRENT**
- Review statement: The phase, capabilities, workstreams, limits, repository-backed deployment claim, M-18 retirement record, completed A01-A08 architecture work, and B01 source-direction guard were reviewed together against the A08 baseline and this worktree's sources, documentation, and focused negative evidence. B01 starts Phase B without restructuring the runtime: it classifies every TypeScript module under src, uses the installed TypeScript compiler API for normal, type-only, re-export, import-type, import-equals, literal dynamic/require, and configured-alias imports, and fails closed for unclassified in-tree modules, non-static module requests, or forbidden reverse dependencies. The finite matrix protects pure domain/contracts/ports/presentation and entry-point ownership. Thirteen pre-B02/B04 application-to-infrastructure composition seams are explicit transitional exceptions, so no new adapter ownership can be introduced silently. B02 still owns TypeScript project references and restricted Core exports; B04 owns composition-root extraction. A08 continues to supply the versioned JSON-safe shared catalog and detached scenario-only conformance seam. No production route, listener, persistence adapter/schema, parser/fetcher, OAuth client, provider enrollment, authentication, billing, hosted repository, service, deployment, live UI, classroom data, or external system changed. Landing-page and synthetic-display worktrees remain separately owned, and no live application, provider, credential, route, or classroom-display probe was performed.
...
- Preserve B01 dependency direction while B02 package restructuring and later operator-panel implementation remain separately scoped and authorized.
...
| `internal-dependency-direction` | Executable internal dependency direction | `implemented` | Classifies all TypeScript source modules and rejects forbidden in-tree layer dependencies before handoff while preserving the current single-package layout. | [`docs/internal-dependency-direction.md`](internal-dependency-direction.md), [`docs/decisions/0026-public-core-and-hosted-shell.md`](decisions/0026-public-core-and-hosted-shell.md), [`docs/core-and-hosted-implementation-work-breakdown.md`](core-and-hosted-implementation-work-breakdown.md) |
...
- Present state: A01 through A08 remain complete, and B01 is the first completed Phase B task. B01 adds the repository-owned architecture direction check without changing runtime behavior or restructuring the application. It classifies every TypeScript file beneath src as contracts, domain, ports, application, infrastructure, presentation, configuration, composition, or entry points; uses TypeScript syntax parsing for ordinary, type-only, re-export, import-type, import-equals, literal dynamic/require, and configured-alias imports; fails closed for unclassified in-tree modules and non-static module requests; and gives deterministic errors for forbidden reverse dependencies. The finite matrix keeps domain/contracts/ports/presentation inward and preserves entry-point ownership. Thirteen existing application-to-infrastructure composition seams are exact transitional exceptions only; B02 must replace reusable shapes with contracts/ports and B04 must extract concrete construction. A08 continues to provide the adapter-free shared conformance seam, but neither it nor B01 threads new contracts into use cases/adapters. No parser, fetcher, OAuth client, production route, live UI, persistence schema, provider enrollment, account flow, hosted repository, billing integration, service, deployment, classroom data, or external system changed.
- Capabilities: `internal-dependency-direction`, `guided-setup`, `preview-and-controls`, `durable-state`, `fixture-evaluation`
- Next: Keep the A08 catalog and privacy inventory versioned whenever a later implementation releases a new display or student field. Use B02 to introduce the reviewed TypeScript project graph and restricted Core exports; it must remove or narrow B01 transitional composition seams rather than treating them as a public boundary.
- Documentation: [`docs/core-and-hosted-feature-acceptance-matrix.md`](core-and-hosted-feature-acceptance-matrix.md), [`docs/core-and-hosted-threat-model.md`](core-and-hosted-threat-model.md), [`docs/core-workspace-actor-contracts.md`](core-workspace-actor-contracts.md), [`docs/core-configuration-state-contracts.md`](core-configuration-state-contracts.md), [`docs/core-source-mode-contracts.md`](core-source-mode-contracts.md), [`docs/core-operator-panel-information-architecture.md`](core-operator-panel-information-architecture.md), [`docs/core-shared-synthetic-fixture-suite.md`](core-shared-synthetic-fixture-suite.md), [`docs/internal-dependency-direction.md`](internal-dependency-direction.md), [`docs/core-and-hosted-implementation-work-breakdown.md`](core-and-hosted-implementation-work-breakdown.md), [`docs/project-state.md`](project-state.md)
...
- Whether B02 should first introduce the reviewed composite project graph and restricted Core exports, or whether a separately authorized implementation slice should first consume the completed A08 shared contract-suite seam.
<!-- prettier-ignore-end -->
