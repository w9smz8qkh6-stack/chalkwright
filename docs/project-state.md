...
- Maintain the resynchronized post-Goal-1 control plane: the WBS and generated queue ledger are the source for dispatch; C10 is accepted, while C11 owns the contact-sheet/carousel UI and B01, C05, C06, C11, and C12 are independently ready.
...
- Present state: Goal 1 is complete and accepted. A07/A08 specify the Core-only information architecture and exact synthetic suite; C01 implements reusable versioned configuration services; C02 supplies the private-listener, no-login operator shell; C03 adds draft room/screen/timezone references and class-code lifecycle; C04 adds the Core source registry and ordinary forms (accepted evidence 564fc70); C09 reconciles the final operator workflow (609abd5 and bbd6392); and C10 independently confirms the full non-creator configuration, preview, activation, rollback, redacted-export, and recovery path (8060793 and b3edda8). C10 does not own the review UI: C11 is the pending contact-sheet, date-picker, enlarged-preview, carousel, and presentation-profile task. The current generated queue reports B01, C05, C06, C11, and C12 ready. D00 has selected Django plus a private versioned Core service/worker boundary through ADR-0027. D07 is separately owned and in progress: its current increment provides hosted Account links to django-allauth security workflows and owner-scoped Organization membership/invitation controls without rendering invitation tokens (0b3ca66). D04 awaits external provider-registration inputs. The Core planning checkout is not evidence of hosted deployment or live operational state.
- Capabilities: `guided-setup`, `preview-and-controls`, `durable-state`, `fixture-evaluation`
- Next: Use the WBS and generated task/execution ledger as the only scheduling authority; run WBS synchronization and freshness checks whenever their source changes. Do not derive status from the canonical checkout's divergence or an agent idle flag. Dispatch B01, C05, C06, C11, or C12 only through their declared dependencies and isolated-worktree ownership. Preserve the C11 contact-sheet/carousel boundary from accepted C10, continue separately owned hosted work under ADR-0027, and resolve D04 provider-registration inputs before connected-provider work.
...
- Which independently ready Core follow-on item—B01, C05, C06, C11, or C12—should be dispatched next under the WBS dependencies and isolated-worktree ownership rules.
