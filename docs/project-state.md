...
- Review statement: The phase, capabilities, workstreams, limits, repository-backed application deployment claim, completed M-18 retirement record, documentation automation, and isolated-worktree guidance were reviewed together against origin/main commit 0f8f65f. The Core operator-panel and exploratory hosted-service roadmap, proposed ADR-0026, 49-task gated work breakdown, and completed A01 feature acceptance matrix are incorporated as an active architecture-planning workstream; implementation has not begun. A02 is next and A03-A08 remain required for the architecture-ready gate. Landing-page and synthetic-display worktrees remain separately owned, and no new live application, provider, route, or service probe was performed.
...
- Advance the Core operator panel through its remaining A02-A08 architecture gate before package restructuring or UI implementation.
...
| `guided-setup` | Guided non-secret installation configuration | `planned` | Provide a private-by-default Core operator panel for versioned configuration, guided sources, readiness, and planned-display review while retaining strict generated runtime contracts. | [`docs/configuration.md`](configuration.md), [`docs/core-and-hosted-feature-acceptance-matrix.md`](core-and-hosted-feature-acceptance-matrix.md), [`docs/future-parity-roadmap.md`](future-parity-roadmap.md), [`docs/core-and-hosted-implementation-work-breakdown.md`](core-and-hosted-implementation-work-breakdown.md), [`docs/decisions/0026-public-core-and-hosted-shell.md`](decisions/0026-public-core-and-hosted-shell.md) |
...
- Present state: A01 is complete: the authoritative feature acceptance matrix freezes owning layers, edition and surface availability, MVP dispositions, explicit exclusions, safety boundaries, observable evidence, downstream task traceability, and unresolved-decision routing. The M-18-aware roadmap, exploratory hosted-service concept, proposed ADR-0026, and 49-task dependency-ordered work breakdown remain the governing planning set. The Core panel remains planned: no package restructuring, operator route, provider enrollment, hosted repository, billing integration, or deployment has begun.
- Capabilities: `guided-setup`, `preview-and-controls`, `durable-state`, `fixture-evaluation`
- Next: Execute A02 to threat-model the frozen surfaces, actors, entry points, abuse cases, mitigations, residual risks, and negative-test requirements. Complete A03 through A08 in dependency order, and do not begin Phase B until the architecture-ready gate is satisfied.
- Documentation: [`docs/core-and-hosted-feature-acceptance-matrix.md`](core-and-hosted-feature-acceptance-matrix.md), [`docs/core-and-hosted-implementation-work-breakdown.md`](core-and-hosted-implementation-work-breakdown.md), [`docs/project-state.md`](project-state.md)
...
- The current strict configuration schemas are not yet the intended guided end-user setup experience; the operator-panel workstream is architecture planning only and has not changed runtime behavior.
...
- What threat-model assets, actors, entry points, abuse cases, mitigations, residual risks, and negative tests A02 should require for the frozen Core and hosted surfaces.
<!-- prettier-ignore-end -->
