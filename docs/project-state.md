...
- Deliver B03 workspace-scope enforcement after accepted B02 Core exports; C05 and C06 remain independently ready but unassigned.
...
- Present state: Goal 1 is complete and accepted. C11/C12 add the accepted review and presentation-profile operator features. B01 is accepted at dd7f0db9052401421a690a6db5e9756ecbbf9445; B02 is accepted at 5c1dc5bbf78797dbd3a4f2ded8daaf1b05feb345 and exposes only deliberate Core package surfaces without changing the existing running application. B03 is the next Core-hardening task; C05 and C06 remain independently ready. D00 selected Django and the private versioned Core service/worker boundary; D07 remains separately in progress and D04 awaits external provider-registration inputs. The planning checkout is not evidence of hosted deployment or live operational state.
- Capabilities: `guided-setup`, `preview-and-controls`, `durable-state`, `fixture-evaluation`
- Next: Use the WBS and generated task/execution ledger as the only scheduling authority; run WBS synchronization and freshness checks whenever their source changes. Do not derive status from the canonical checkout's divergence or an agent idle flag. Dispatch B03 workspace-scope enforcement in an isolated worktree, then choose the next ready item after its acceptance. Continue separately owned hosted work under ADR-0027, and resolve D04 provider-registration inputs before connected-provider work.
