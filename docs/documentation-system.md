...
## Cross-repository landing-page handoff

The public landing page lives in `/home/bren/chalkwright-site`, but Chalkwright's
canonical operational and semantic account lives in this repository. Every
task that changes landing-page source, media or player behavior, dependencies,
Git topology, hosting configuration, publication state, runtime security, or a
verified live-state claim must update the governing application documentation
before that same task is complete. No follow-up documentation request is
required.

At minimum, review `docs/landing-page-runbook.md` and this repository's
Unreleased changelog. Also update `docs/homepage-demo-video.md` for media or
player changes, `.codex/environment.md` for stable topology or runtime facts,
and `docs/project-knowledge.json` plus `docs/project-state.md` when the change
alters semantic status, an active workstream, capabilities, limits, or next
decisions. The landing repository's README, guidance, and changelog remain part
of the same review. Generated refreshes handle derived facts; Codex must still
judge and write the explanatory meaning. If either repository cannot be
updated accurately, handoff is blocked and the exact gap must be reported.
...
views. They remain reviewable changes and are never committed automatically.
