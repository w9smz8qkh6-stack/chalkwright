...
<!-- project-drive-workspace -->

## Shared temporary workspace

- [Google Drive temp workspace](https://drive.google.com/drive/folders/1OHOLkhXQAbx9rLJrCoPr70JTQrJGwNot) is an auxiliary, as-needed workspace for temporary shared artifacts. It does not replace the GitHub repository or canonical project storage.
...
- The public landing-page source is the separate local repository at
  `/home/bren/chalkwright-site`. Read `docs/landing-page-runbook.md` and that
  repository's `AGENTS.md` before changing it.
...
- B407 is a synthetic fixture. C509 is the current production-validation
  ...
  `Asia/Ho_Chi_Minh`.
- Before host, deployment, landing-page, media-render, or live-service work,
...
- A task that changes the separate landing page at
  `/home/bren/chalkwright-site` must perform its application-repository
  documentation handoff in the same task without waiting for a separate user
  request. Review and update `docs/landing-page-runbook.md`,
  `docs/homepage-demo-video.md` when media or player behavior changes,
  `.codex/environment.md` when stable topology or runtime facts change, the
  Unreleased changelog, and `docs/project-knowledge.json` plus its generated
  state when semantic status, workstreams, or decisions change. If the
  application repository is unavailable or conflicting work prevents an
  accurate update, the landing-page task is incomplete and must report the
  exact documentation gap.
...
- For landing-page changes, follow `docs/landing-page-runbook.md`: run its
  repository check, inspect desktop and mobile layouts, and verify navigation,
  metadata, reduced motion, captions, media range requests, and MIME types.
...
- Landing page: `docs/landing-page-runbook.md`
- Demo media: `docs/homepage-demo-video.md` when present
