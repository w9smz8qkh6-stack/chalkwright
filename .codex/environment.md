...
- `npm run docs:check`: `npm run environment:check --silent && node scripts/update-tooling-index.mjs --check && node scripts/check-local-doc-links.mjs && node scripts/check-changelog.mjs && npm run wbs:check --silent`
- `npm run docs:sync`: `node scripts/update-tooling-index.mjs --write && npm run environment:sync --silent && npm run wbs:sync --silent`
