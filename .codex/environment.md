...
Last curated review: 2026-08-31
...
- Public landing-page repository: `/home/bren/chalkwright-site`
- Canonical landing-page remote:
...
The landing-page folder should also be attached to the Chalkwright local
project in the Codex app. Until it is attached, the absolute path above is the
explicit discovery route; secondary project folders do not replace this
repository's primary instruction discovery.
...
- Canonical source remote:
  `https://github.com/w9smz8qkh6-stack/chalkwright-site.git`
- Default source branch: `main`
- Runtime: Docker Compose service and container `chalkwright-site`
- Origin binding: `127.0.0.1:18083` to container port 3000
- Public canonical origin: `https://chalkwright.org`
- Public transport: Cloudflare Tunnel to the loopback origin

The 2026-08-31 landing-page publication is recorded at source commit
`ef120516f84fc31c9c59b29a9b09e4b3aeaa6d1c`. Pushing the landing-page remote
does not deploy it; live publication remains an explicitly authorized local
Docker Compose operation.

The domain registrar, authoritative DNS, Cloudflare proxy/tunnel, and Lenovo
origin are separate control planes. Verify each layer instead of inferring
hosting from the registrar or from `.openai/hosting.json`. That file describes
build-tool metadata; it is not evidence that the live site is hosted by OpenAI
Sites.

Do not restart `cloudflared`, alter its configuration, rebuild the container,
or publish the site unless the current request explicitly authorizes that
effect. Follow `docs/landing-page-runbook.md` for checks and rollback planning.
